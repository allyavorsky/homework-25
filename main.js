...
function setUserInfoCookie(key, value) {
  ...
  console.log(`Cookie userInfo "${cookieValue}" set as 10 second`);
}
...
function saveUserInfo(key, value) {
  ...
  console.log(`Saved ${key}: ${value}`); // Saved username: JohnDoe
}

function getUserInfo(key) {
  const value = sessionStorage.getItem(key);
  console.log(`Retrieved ${key}: ${value}`); // Retrieved username: JohnDoe
  return value;
}

// Демонстрація використання функцій
saveUserInfo("username", "JohnDoe");
console.log(getUserInfo("username")); // JohnDoe
